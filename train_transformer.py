#!/usr/bin/env python3
"""Train Transformer model for sentiment analysis with optimized settings."""
...
def main():
    """Main training function for optimized Transformer model."""
    parser = argparse.ArgumentParser(description="Train optimized Transformer model for sentiment analysis")
    ...
    # Create Transformer model with optimized settings
    logger.info("Creating optimized Transformer model...")
    ...
    # Set up training components with Transformer-optimized settings
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.AdamW(model.parameters(), lr=args.learning_rate, weight_decay=0.01)
    scheduler = create_scheduler(optimizer, name="cosine", T_max=args.epochs)
    early_stopping = EarlyStopping(patience=4, min_delta=0.005, mode="max")
    ...
    # Train the model
    logger.info("Starting optimized Transformer training...")
    history = trainer.fit(
        epochs=args.epochs,
    ...
    # Save final model
    checkpoint_path = checkpoint_dir / "transformer_optimized.pt"
    ...
    history_path = checkpoint_dir / "transformer_optimized_history.yaml"
